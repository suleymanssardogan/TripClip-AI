...
def test_delete_itinerary_requires_auth(client):
    resp = client.delete("/api/mobile/itineraries/4")
    assert resp.status_code == 403
...
def test_delete_itinerary_happy_path(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.delete("/api/mobile/itineraries/4", headers=auth_headers)
    assert resp.status_code == 200
...
def test_delete_itinerary_propagates_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.delete("/api/mobile/itineraries/999", headers=auth_headers)
    assert resp.status_code == 404
    assert resp.json()["code"] == "ITINERARY_NOT_FOUND"


def test_delete_itinerary_propagates_upstream_internal_error_as_500(client, auth_headers, mock_core_api, make_response):
    mock_core_api.delete.return_value = make_response(
        500, {"error": {"code": "INTERNAL_SERVER_ERROR", "message": "boom"}}
    )
    resp = client.delete("/api/mobile/itineraries/4", headers=auth_headers)
...
def test_list_apply_history_requires_auth(client):
    resp = client.get("/api/mobile/trips/1/itinerary-apply-history")
    assert resp.status_code == 403


def test_undo_apply_history_requires_auth(client):
    resp = client.post("/api/mobile/trips/1/itinerary-apply-history/1/undo")
    assert resp.status_code == 403
...
def test_list_apply_history_happy_path(client, auth_headers, mock_core_api, make_response):
    mock_core_api.get.return_value = make_response(200, {"entries": [
        ...
        {"id": 1, "itinerary_id": 4, "itinerary_created_at": "2026-08-10T09:00:00", "is_undo": False,
         "applied_at": "2026-08-10T10:00:00", "actor_user_id": 1, "is_undoable": False},
    ]})

    resp = client.get("/api/mobile/trips/1/itinerary-apply-history", headers=auth_headers)
    assert resp.status_code == 200
...
def test_list_apply_history_propagates_trip_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.get("/api/mobile/trips/999/itinerary-apply-history", headers=auth_headers)
    assert resp.status_code == 404
    assert resp.json()["code"] == "TRIP_NOT_FOUND"
...
def test_undo_apply_history_happy_path(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post("/api/mobile/trips/1/itinerary-apply-history/2/undo", headers=auth_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert data["history_id"] == 3
    assert data["itinerary_id"] == 4
...
def test_undo_apply_history_propagates_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post("/api/mobile/trips/1/itinerary-apply-history/999/undo", headers=auth_headers)
    assert resp.status_code == 404
...
def test_undo_apply_history_propagates_upstream_error_as_500(client, auth_headers, mock_core_api, make_response):
    mock_core_api.post.return_value = make_response(
        500, {"error": {"code": "INTERNAL_SERVER_ERROR", "message": "boom"}}
    )
    resp = client.post("/api/mobile/trips/1/itinerary-apply-history/2/undo", headers=auth_headers)
