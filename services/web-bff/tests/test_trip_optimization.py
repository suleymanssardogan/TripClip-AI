"""
Web BFF — AI Trip Optimizer proxy testleri.
...
def test_optimize_requires_auth(client):
    resp = client.post("/api/web/trips/1/optimize", json={"selected_place_ids": [1]})
    assert resp.status_code == 401


def test_list_itineraries_requires_auth(client):
    resp = client.get("/api/web/trips/1/itineraries")
    assert resp.status_code == 401


def test_get_itinerary_requires_auth(client):
    resp = client.get("/api/web/itineraries/1")
    assert resp.status_code == 401
...
def test_list_itineraries_happy_path(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.get("/api/web/trips/1/itineraries", headers=auth_headers)
    assert resp.status_code == 200
...
def test_get_itinerary_happy_path(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.get("/api/web/itineraries/4", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["id"] == 4
...
def test_optimize_forwards_all_optional_fields(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post("/api/web/trips/1/optimize", json=body, headers=auth_headers)
    assert resp.status_code == 200
...
def test_optimize_defaults_match_core_api_defaults(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post(
        "/api/web/trips/1/optimize", json={"selected_place_ids": [1]}, headers=auth_headers,
    )
    assert resp.status_code == 200
...
def test_optimize_response_body_passthrough_unchanged(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post(
        "/api/web/trips/1/optimize", json={"selected_place_ids": [12]}, headers=auth_headers,
    )
...
def test_optimize_propagates_invalid_request_error(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post(
        "/api/web/trips/1/optimize", json={"selected_place_ids": []}, headers=auth_headers,
    )
...
def test_optimize_propagates_trip_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post(
        "/api/web/trips/999/optimize", json={"selected_place_ids": [1]}, headers=auth_headers,
    )
...
def test_get_itinerary_propagates_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.get("/api/web/itineraries/999", headers=auth_headers)
    assert resp.status_code == 404
...
def test_list_itineraries_propagates_trip_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.get("/api/web/trips/999/itineraries", headers=auth_headers)
    assert resp.status_code == 404
    assert resp.json()["code"] == "TRIP_NOT_FOUND"


def test_optimize_propagates_upstream_internal_error(client, auth_headers, mock_core_api, make_response):
    """core-api'nin INTERNAL_SERVER_ERROR'ı web-bff'de 503'e eşlenir — bkz.
    error_wrapper.py'nin DATABASE_ERROR/ML_SERVICE_UNAVAILABLE ile aynı
    "sunucu hatası" grubu (mobile-bff'nin aksine, web bu grubu 500 değil
    503 döner — kullanıcıya 'tekrar dene' çağrısı yapan bir mesajla)."""
    mock_core_api.post.return_value = make_response(
...
def test_apply_itinerary_requires_auth(client):
    resp = client.post("/api/web/itineraries/4/apply")
    assert resp.status_code == 401
...
def test_apply_itinerary_propagates_not_found_as_404(client, auth_headers, mock_core_api, make_response):
    ...
    resp = client.post("/api/web/itineraries/999/apply", headers=auth_headers)
    assert resp.status_code == 404
    assert resp.json()["code"] == "ITINERARY_NOT_FOUND"
